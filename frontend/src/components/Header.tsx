...
import { FamilyMember, SubscriptionStatus } from '../lib/types';
import { getCurrentUser, setCurrentUser, loadMembers, loadSubscription } from '../lib/storage';
import { Sparkles, Shield, ShieldAlert, ChevronDown, Check, Crown, Activity } from 'lucide-react';

interface HeaderProps {
  onOpenPremium: () => void;
}

export const Header: React.FC<HeaderProps> = ({ onOpenPremium }) => {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'home':
        return <span className="inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded-full bg-emerald-500/20 text-emerald-300 border border-emerald-500/30">🏠 Zuhause</span>;
      case 'away':
        return <span className="inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded-full bg-amber-500/20 text-amber-300 border border-amber-500/30">🚗 Unterwegs</span>;
      case 'work':
        return <span className="inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded-full bg-blue-500/20 text-blue-300 border border-blue-500/30">💼 Büro</span>;
      case 'school':
        return <span className="inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded-full bg-purple-500/20 text-purple-300 border border-purple-500/30">📚 Schule</span>;
      default:
        return <span className="inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded-full bg-slate-500/20 text-slate-300 border border-slate-500/30">📍 Aktiv</span>;
    }
  };
    ...
    <header className="sticky top-0 z-30 w-full glass-panel border-b border-white/10 px-4 py-3 sm:px-6">
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <div className="relative flex items-center justify-center w-10 h-10 rounded-xl bg-gradient-to-tr from-emerald-600 to-teal-500 shadow-lg shadow-emerald-500/25">
        ...
        <div className="flex items-center gap-2 sm:gap-4">
          ...
          <div className="relative">
            ...
            {memberMenuOpen && (
              <div className="absolute right-0 mt-2 w-56 rounded-2xl glass-panel bg-slate-900/95 border border-white/15 shadow-2xl py-2 z-50 animate-in fade-in slide-in-from-top-2 duration-150">
                <div className="px-3 py-1.5 border-b border-white/10">
                  <p className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider">Aktives Profil wechseln</p>
                </div>
